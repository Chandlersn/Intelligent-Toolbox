...
两件事正在同时发生：一边是 AI 的能力爆发，大模型能总结、能关联、能洞察，理应成为每个人的思考外脑；另一边是个人数据的彻底碎片化，你的信息散在十几个 App 里，取不出来，也聚不起来。

结果是一个尴尬的错位：**AI 很强，但它不认识你。** 你问它任何问题，它只能回一个「对所有人都成立」的答案。不是它不行，是它拿不到关于你的数据——在它眼里，你是个陌生人。
...
**① 把「存了不看」变成「存了就有用」**
你不用回看，也不用整理。收进来的东西由后台自动分类、归位、建立关联——**收藏不再是终点，而是一次认知网络的节点登记**。你只管收，剩下的交给它；人越懒，数据反而越全。

**② 把碎片聚成一张网**
不管你从抖音、GitHub 还是文章里收，都落进**同一张多轴图谱**：按领域、技术栈、用途、动机投影成节点，共同轴上相邻的自动连线。跨来源的东西会在共同主题上相遇——尤其是**生产端（你做的）** 和 **认知端（你学的）** 之间的连线，会被单独标出来。

**③ 让数据回到你手里，也喂得起 AI**
数据只存本机 SQLite，默认不出本机。而一旦它结构化了，就能反过来被 AI 调用：接上你自己的 LLM，卡片分析、推荐、画像都会基于**你真实的行为数据**生成——这时候 AI 给出的，才是「只对你成立」的答案。
...
| **认知端** | 抖音视频、网页 / 文章链接 | **原文逐字稿 / 正文片段** + 萃取卡片，两份都留 |
...
| 多源·扩展 | 新增网页 / 文章来源（认知端第 4 源）：标准库抓取标题与正文片段，与仓库在共同领域轴上碰撞 |
| 收敛 | 全站页面标题简化、卡片操作区收敛（7 按钮 → 3 + 1）、分享页与画像页重做——信息密度降下来，操作更顺 |
